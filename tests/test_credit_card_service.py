...
async def test_create_credit_card(mock_session):
    ...
    with patch('app.services.credit_card_service.encrypt_credit_card_info', side_effect=lambda x: f"encrypted_{x}"):
        credit_card = await CreditCardService.create_credit_card(mock_session, card_info)
...
async def test_get_credit_card(mock_session):
    ...
    with patch('app.services.credit_card_service.decrypt_credit_card_info', side_effect=lambda x: x.replace("encrypted_", "")):
...
async def test_update_credit_card(mock_session):
    ...
    with patch('app.services.credit_card_service.encrypt_credit_card_info', side_effect=lambda x: f"encrypted_{x}"):
        result = await CreditCardService.update_credit_card(mock_session, card_id, card_info)
